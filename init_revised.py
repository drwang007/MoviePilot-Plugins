...
class ANiStrm(_PluginBase):
    ...
    def get_current_season_list(self) -> List:
        ...
        # API返回的是JSON，通常包含files列表
        if rep.status_code == 200:
             try:
                 files_json = rep.json()['files']
                 return [file['name'] for file in files_json]
             except Exception as e:
                 logger.error(f"解析当季列表失败: {e}")
                 return []
        else:
             logger.warn(f"获取当季列表失败，状态码: {rep.status_code}")
             return []
    ...
    def __touch_strm_file(self, file_name, file_url: str = None) -> bool:
        if not file_url:
            # 季度API生成的URL，使用新格式
            # [修复2] 处理文件名，防止出现 .mp4.mp4
            clean_name = file_name
            if clean_name.endswith('.mp4'):
                # 移除后缀以便下面统一拼接，或者直接使用原名不拼接后缀
                # 这里选择移除后缀重新拼接，保持原有逻辑结构
                encoded_filename = quote(clean_name[:-4], safe='')
            else:
                encoded_filename = quote(clean_name, safe='')
            
            # 确保 self._date 存在，如果不存在重新获取
            if not self._date:
        ...
        if not os.path.exists(self._storageplace):
            try:
                ...
                logger.error(f"创建目录失败: {self._storageplace}, 错误: {e}")
                return False

        file_path = f'{self._storageplace}/{file_name}.strm'
        if os.path.exists(file_path):
            # logger.debug(f'{file_name}.strm 文件已存在') # 减少日志垃圾
            return False
        try:
    ...
    def __task(self, fulladd: bool = False):
        ...
        if not fulladd:
            ...
            logger.info(f'ANiStrm: 检测到RSS更新，本次处理 {len(rss_info_list)} 个文件')
        ...
        # 全量添加当季
            ...
            logger.info(f'ANiStrm: 执行全量补全，本次处理 {len(name_list)} 个文件')
            for file_name in name_list:
                # 全量添加时不传 url，触发内部自动构建逻辑
                if self.__touch_strm_file(file_name=file_name):
                    cnt += 1
        if cnt > 0:
            logger.info(f'ANiStrm: 新创建了 {cnt} 个strm文件')
